...
Ref<Mesh> Plane::create(const Ref<Renderer>& renderer, const Material& material) {
    ...
    const std::array<unsigned int, 6> indexes = {0, 1, 2, 2, 3, 0};
    const auto vertexDataFloats = reinterpret_cast<const float*>(vertices);
    return std::make_shared<Mesh>(renderer, vertexDataFloats, sizeof(vertices), indexes.data(), indexes.size(), material, glm::mat4(1.0f));
}
