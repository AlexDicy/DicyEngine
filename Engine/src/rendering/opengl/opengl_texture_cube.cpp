...
Ref<TextureCube> OpenGLTextureCube::create_from_hdr(const Ref<Renderer>& renderer, const Ref<Texture2D>& hdr_texture, const Ref<Shader>& convert_shader, uint32_t size) {
    ...
    const SkyboxCube cube(renderer, convert_shader, hdr_texture);
    for (unsigned int i = 0; i < 6; i++) {
